...
public class AnimationHierarchyEditor : EditorWindow {
	...
	void OnGUI()
    {
        ...
        using (new GUILayout.VerticalScope("helpbox"))
        {
            ...
            animationObject = (Animation) EditorGUILayout.ObjectField("Root Animation:", animationObject, typeof(Animation), true);
    ...
    private GameObject FindObjectInRoot(string path)
    {
        if (animatorObject)
        {
            return animatorObject.transform.Find(path)?.gameObject;
        } else if(animationObject) {
            return animationObject.transform.Find(path)?.gameObject;
        }
        else
        {
            return null;
    ...
    {
        if (animatorObject == null && animationObject == null) throw new UnityException("Please assign the Root Animator first!");
        if(animatorObject){
            ...
            if (!obj.transform.IsChildOf(animationObject.transform)) throw new UnityException($"Object must belong to {animationObject.name} !");
            return AnimationUtility.CalculateTransformPath(obj.transform, animationObject.transform);
        }
    }
